...
import { UpdatedParamters } from "./WebGLRenderer";
...
export class MeshRender {
    ...
    updateMaterialParameters(parameters: UpdatedParamters) {
        ...
        for (let k in this.material.uniforms) {
            if (k in parameters) {
                const j = k as keyof UpdatedParamters;
                this.material.uniforms[k].value = parameters[j];
            }
        }
    }
